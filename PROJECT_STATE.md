...
| CI | **LIVE, green since 2026-09-16** | `.github/workflows/ci.yml`, running against `mylestattooer-rgb/PRIVATE` on GitHub since the 2026-08-14 repo extraction (see "Repo extraction" below). **It had never actually passed** — all three runs failed on `npx tsc --noEmit` with `app/layout.tsx(20,50): error TS2304: Cannot find name 'LayoutProps'`, and since the workflow runs under `bash -e`, `npm run test` never executed at all. Root cause: Next 16 generates the route type helpers (`LayoutProps`/`PageProps`/`RouteContext`) into `.next/types`, which `tsconfig.json` includes, and those only appeared after `next dev`/`next build` — neither of which CI runs. Fixed by adding `npx next typegen` before the typecheck, the pattern Next's own CLI docs prescribe for this exact case. Local repro and verification: clean `.next` -> tsc fails; `next typegen` -> tsc exits 0 |
...
## Simulator domain (new, 2026-09-16)

`app/lib/domains/simulator/` — Phase 6's deterministic core, built as pure domain logic with no
Prisma, Next or network imports. 144 tests (full suite 218/218), `tsc --noEmit` and `eslint` clean,
and `npm run backtest` produces a real report against seeded synthetic data or a CSV.

The shape is the one `AI_ARCHITECTURE.md` already specified for the trading side: Signal →
Risk Manager → Execution Adapter → Portfolio → Metrics. What makes it more than a diagram is that
the boundary is typed — a `Signal` has no `quantity` field, so an AI-produced signal physically
cannot express a position size, and `risk.ts` is the only module that can turn one into an Order.
`parseSignal()` is the single door untrusted model output comes through: it pins the symbol to the
one requested, assigns provenance from the caller rather than the payload, drops invented fields,
and degrades to `hold` on anything malformed.

No-look-ahead is structural rather than conventional (`PRODUCT_SPEC.md` names it as a Simulator
requirement): a strategy deciding on bar `i` gets a frozen copy of bars `0..i`, and orders execute
at bar `i+1`'s open. A test runs a strategy that only buys when tomorrow closes higher against a
fixture where every bar closes higher — it takes zero trades.

**Deliberately not built, and why**: no `SimulatorSession`/`SimulatorTrade` persistence yet
(`DATABASE.md` §2.6 wants `dataSourceId` provider-agnostic, and the data-provider question is
still open — the engine is designed around that decision not being made yet); no student UI or
`USE_SIMULATOR` entitlement; no live-broker adapter, which `PRODUCT_SPEC.md`'s permanent non-goal
puts outside this application entirely. `runBacktest` throws on any adapter declaring
`isLive: true`, so the boundary fails loudly rather than silently. See
`app/lib/domains/simulator/README.md` for the full model limitations and live-adapter prerequisites.
